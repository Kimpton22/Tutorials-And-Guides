...
3. Next, open the ```.log``` file in bash and type ```/thermal```. This does a forward search (like ctrl+f) of the file for the keyword 'thermal'. This will bring you to the part of the output where the energies are printed. Usually gibbs free energy is the best value to take, but Gaussian also provides electronic and enthalpic energies. If you have multiple outputs that you want to analyze at the same time, try using this command: ```grep -m3 'thermal' *.log >> energies.txt```. This will go through all ```.log``` files and search for the first 3 occuraces of thermal and print them to a text file.
