...
l = len(sys.argv)
if l < 2:
  print('Usage: '+os.path.basename(sys.argv[0])+' filename.com ncpus(=16) memory(=16) NOTE: WALLTIME IS 23 HOURS (limit on short)')
  sys.exit(1)
...
def write_bash_file(bash_filename, g16_filename, filename):
    ...
    bash_file.write('#SBATCH --partition=short\n')
    bash_file.write('#SBATCH --time='+str(walltime)+':00:00\n')
